...
class cell():
  """ Initilalizes cell object. A cell is a single box of a sudoku puzzle. 81 cells make up the body of a sudoku puzzle. Initializes puzzle with all possible answers available, solved to false, and position of cell within the sudoku puzzle"""
  ...
  def setAnswer(self, num):
    """ Sets an answer of a puzzle and sets a cell's solved method to true. This method also eliminates all other possible numbers"""
...
def sudokuGen():
  ...
  cells = [i for i in range(81)] ##? cells is the positions of cells not currently set
  sudoku = emptySudoku()
  while len(cells) != 0:
    lowestNum = []
    Lowest = []
    ...
    '''Randomly choose a possible answer and set it to the cell'''
...
def sudokuChecker(sudoku):
  """ Checks to see puzzle is of the correct format and abides by all of the rules of a sudoku. Returns True if the puzzle is correct. False if otherwise"""
...
def perfectSudoku():
  '''Generates a completely random completed sudoku in the correct formet. '''
  result = False
  while result == False:
    s = sudokuGen()
...
def solver(sudoku, f = 0):
  ...
  cells = [i for i in range(81)] ##? our cells is the position of the cells not currently set
  solvedCells = []
  for i in cells:
...
def solve(sudoku, n = 0):
  """ Uses the solver method to solve a puzzle. This method was built in order to avoid recursion depth errors. Returns True only if the puzzle is solvable."""
  if n < 30:
    s = solver(sudoku)
    if s != False:
      return s
...
def main(level):
  """ Input the difficulty level of the sudoku puzzle. Difficulty levels include ‘Easy’ ‘Medium’ ‘Hard’ and ‘Insane’. Outputs a sudoku of desired difficulty. """
  t1 = time.time()
  n = 0
  if level == 'Easy':
    p = perfectSudoku()
